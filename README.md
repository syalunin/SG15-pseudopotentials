...
This repository contains the SG15 Optimised Norm-Conserving Vanderbilt (ONCV) pseudopotentials, generated using a modified version of the scalar-relativistic code by [D. R. Hamann](http://www.quantum-simulation.org/potentials/sg15_oncv/). I modified the code to ensure that the pseudopotentials now include the wavefunctions of the core states for all atoms. Except for this, these pseudopotentials are identical to those obtained by M. Schlipf and F. Gigi, now known as the SG15 collection.

The core wavefunctions are written in a separate section of the pseudopotential file in GIPAW format, enabling Quantum ESPRESSO to read and use them in calculations. Theoretically, these wavefunctions can be used to calculate NMR shielding tensors, EFG tensors, and other magnetic resonance properties using, for example, the [QE-GIPAW package](https://github.com/dceresoli/qe-gipaw), but I cannot guarantee full compatibility.
