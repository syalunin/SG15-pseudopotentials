...
!
 ...
 real(dp), allocatable :: uu(:),uu2(:),up(:),up2(:),uuc(:,:)
...
!calculate zion for output
 ...
 allocate(uu(mmax),uu2(mmax),up(mmax),up2(mmax),uuc(mmax,nc))
...
!
! calculate all-electron core orbitals for GIPAW
 do ii=1,nc
   call lschfb(na(ii),la(ii),ierr,ea(ii), &
&              rr,vfull,uu,uu2,zz,mmax,mch,srel)
   uuc(:,ii)=uu(:)/rr(:)
 end do
!
...
&               rr,vfull,vp,zz,mmax,irpsh,srel)
 ...
 if(index(psfile,'psp8') > 0) then
  call linout(lmax,lloc,rc,vkb,evkb,nproj,rr,vpuns,rho,rhomod, &
&             zz,zion,mmax,iexc,fcfact,nrl,drl,atsym)
 end if
 if(index(psfile,'upf') > 0) then
  call upfout(lmax,lloc,rc,vkb,evkb,nproj,rr,vpuns,rho,rhomod,rhoc,uuc, &
&             zz,zion,mmax,iexc,icmod,nrl,drl,atsym,epstot, &
&             na,la,ncon,nbas,nvcnf,nacnf,lacnf,nc,nv,lpopt,ncnf, &
&             fa,rc0,ep,qcut,debl,facnf,dvloc0,fcfact, &
&             epsh1,epsh2,depsh,rlmax,'upf ')
 end if
 if(index(psfile,'fpmd') > 0) then
  call fpmdout(lmax,lloc,rc,vkb,evkb,nproj,rr,vpuns,rho,rhomod, &
&              zz,zion,mmax,iexc,icmod,nrl,drl,atsym,epstot, &
&              na,la,ncon,nbas,nvcnf,nacnf,lacnf,nc,nv,lpopt,ncnf, &
&              fa,rc0,ep,qcut,debl,facnf,dvloc0,fcfact, &
&              epsh1,epsh2,depsh,rlmax,'fpmd')
 end if
