...
!deriv_method = XC_DERIV_ANALYTICAL
 ...
 call functional_null(functls(1))
 call functional_null(functls(2))
 call functional_init(nspin, id(1), deriv_method, irel, functls(1))
 call functional_init(nspin, id(2), deriv_method, irel, functls(2))
 
 if (functprinted==0) then
   write (*,*) 'functional part 1 ', trim(functional_name(functls(1)))
   write (*,*) 'functional part 2 ', trim(functional_name(functls(2)))
   write (*,*)
   functprinted = 1
 end if
...
! ip is an ionization potential parameter for certain xc functionals, not supported here. Set to 0.5 Ha
 ...
 do ifunc = 1, 2
   v = 0.0d0
   e = 0.0d0
   call functional_get_vxc(functls(ifunc), mmax, al, rr, rho_loc, dpr, dlap, &
