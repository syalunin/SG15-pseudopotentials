...
&       stop 'error deriv_method not well defined'
...
stop 'error - parsing of xalpha coefficient not coded'
  ...
  subroutine functional_get_vxc(functl, np, al, rr, rho, rho_grad, rho_lapl, tau, ip, v, e, vtau)
    ...
    if (.not. (size(v, dim=2) == functl%nspin)) stop 'error bad nspin definition'
    if (.not. (functional_kind(functl) /= XC_KINETIC)) stop 'error no kinetic functional here'
...
!          v(:, is) = v(:, is) - mesh_divergence(m, dedgrad(:, is))
      ...
      elseif (functl%deriv_method == XC_DERIV_ANALYTICAL) then
        stop 'error - no analytical derivatives coded'
      end if
    end if

    ! MGGA contribution
    if (functl%family == XC_FAMILY_MGGA) then
stop 'error : meta GGA not coded yet '
      if (functl%deriv_method == XC_DERIV_NUMERICAL) then
        allocate (dpr(np))
        allocate (dppr(np))
        allocate (dlap(np))
...
!          v(:, is) = v(:, is) + mesh_laplacian(m, dedlapl(:, is))
  ...
  subroutine functional_get_tau(functl, np, rho, rho_grad, rho_lapl, tau)
    ...
    if (.not. (functional_kind(functl) == XC_KINETIC)) stop 'error no kinetic part here'
